...
// Get distinct systems for page rendering
...
// API route to get sub-systems for a given system
...
// API route to get symptoms for a given sub-system
router.get('/api/symptoms/:subSystem', async (req, res) => {
  try {
    ...
    const symptoms = rows.map(r => r.symptoms);
    res.json(symptoms);
  } catch (err) {
